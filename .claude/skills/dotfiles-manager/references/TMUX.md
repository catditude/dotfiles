...
- **Inactive-pane dimming**: `window-style` / `window-active-style` set the pane's default fg/bg (inactive `fg=#d0d0d0,bg=#121212`, active `fg=#ffffff,bg=#1c1c1c` — the bg carries most of the dim, the fg only a little). They're *window* options — verify with `tmux show -gw`, not `show -g`. Apps that paint their own background (nvim, less with a theme) override the dim; it mainly shows on shell panes.
...
- **The walker skips the pane you're on.** Claude badges the pane you're watching, so the current pane is often the *newest* badge — and landing on it changes no focus, fires no `pane-focus-in`, clears nothing, so every press re-picks it and the rest of the queue becomes unreachable. Excluding it leaves the badge pending; returning later is a real focus change and clears it normally.
